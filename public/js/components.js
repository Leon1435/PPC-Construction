...
(function () {
  ...
  function ensureStickySocial(company) {
    const existing = document.querySelector('[data-sticky-social]');
    if (existing) return;
    ...
    const fbUrl = String(company.facebookUrl || '').trim();
    const waUrl = String(whatsappHref || '').trim();

    if (!fbUrl && !waUrl) return;
    ...
    const socialFacebook = fbUrl
      ? `
          <a href="${escapeAttr(fbUrl)}" class="sticky-social__icon sticky-social__icon--facebook" target="_blank" rel="noopener noreferrer" aria-label="Facebook">
            ${fbSvg}
          </a>`
      : '';

    const socialWhatsApp = waUrl
      ? `
          <a href="${escapeAttr(waUrl)}" class="sticky-social__icon sticky-social__icon--whatsapp" target="_blank" rel="noopener noreferrer" aria-label="WhatsApp">
            ${waSvg}
          </a>`
      : '';

    const wrap = document.createElement('div');
    wrap.setAttribute('data-sticky-social', 'true');
    wrap.className = 'sticky-social';
    wrap.innerHTML = socialWhatsApp + socialFacebook;
